...
public class NPMovingAverageStepDetector extends NPStepDector {
	...
	private NPMovingAverageTD[] MAFilters;
	private NPCumulativeSignalPowerTD CSP_Filter;
	...
	public NPMovingAverageStepDetector(Context context) {
		this(context, MA1_WINDOW, MA2_WINDOW, POWER_CUTOFF_VALUE);
	}

	public NPMovingAverageStepDetector(Context context, double windowMA1,
		...
		mMASwapState = true;
		MAFilters = new NPMovingAverageTD[] { new NPMovingAverageTD(mWindowMA1),
				new NPMovingAverageTD(mWindowMA1),
				new NPMovingAverageTD(mWindowMA2) };
		CSP_Filter = new NPCumulativeSignalPowerTD();
		mStepDetected = false;
	...
	private void processAccelerometerValues(long timestamp, float[] values) {
		...
		if (mStepDetected && !mSignalPowerCutoff) {
			...
			notifyOnStepEvent(new NPStepEvent(1.0, mStrideDuration));
		}
	}
}
